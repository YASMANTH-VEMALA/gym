...
import { useMembershipPlanMutation } from './use-membership-plans';
export function ArchivePlanDialog({ id, name }: { id: string; name: string }) {
  const [open, setOpen] = useState(false);
  const mutation = useMembershipPlanMutation('archive', id);
  return (
    <Dialog.Root
      open={open}
      onOpenChange={(value) => {
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 max-h-[90dvh] w-[calc(100%-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 space-y-4 overflow-y-auto rounded-xl bg-white p-6 shadow-xl">
          ...
          <Dialog.Description className="text-sm leading-6 text-slate-600">
            Archiving this plan prevents it from being assigned to new
            memberships. Existing historical memberships will remain unchanged.
          </Dialog.Description>
          {mutation.error && (
