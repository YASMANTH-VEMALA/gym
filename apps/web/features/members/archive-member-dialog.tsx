...
import { useMemberMutation } from './use-members';
export function ArchiveMemberDialog({
  ...
  const mutation = useMemberMutation('archive', id);
  return (
    <Dialog.Root
      open={open}
      onOpenChange={(value) => {
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 max-h-[90dvh] w-[calc(100%-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 space-y-4 overflow-y-auto rounded-xl bg-white p-6 shadow-xl">
          ...
          <Dialog.Description className="text-sm leading-6 text-slate-600">
            Archiving this member removes them from normal operational lists
            while preserving their historical records.
          </Dialog.Description>
          {mutation.error && (
